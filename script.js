class MedicalScheduler {
    constructor() {
        ...
        this.btnLogout = document.getElementById('btn-logout'); // Botão de sair
    ...
    init() {
        ...
        if(greetingElement) greetingElement.textContent = `Olá, ${userName}`;
    ...
    handleSubmit(event) {
        ...
        const dados = {
            ...
            email: this.escapeHtml(document.getElementById('emailPaciente').value),
    ...
    prepararEdicao(id) {
        ...
        if(item) {
            ...
            document.getElementById('emailPaciente').value = item.email;
    ...
    render() {
        ...
        listaFiltrada.forEach(item => {
            ...
            li.innerHTML = `
                <article class="appointment-card border-${item.tipo}">
                    <div class="appointment-header">
                        <div class="appointment-title">
                            <h3>${item.nome}</h3>
                            <span class="appointment-type type-${item.tipo.toLowerCase()}">${item.tipo}</span>
                        </div>
                    </div>
                    <div class="appointment-body">
                        <p><strong>Data:</strong> ${dataFormatada} às ${item.hora}</p>
                        <p><strong>Contato:</strong> ${item.telefone}</p>
                        <p><strong>E-mail:</strong> ${item.email}</p>
                    </div>
                    ${item.obs ? `<div class="appointment-notes"><p>${item.obs}</p></div>` : ''}
                    <div class="appointment-actions">
                        <button class="btn btn-small btn-edit btn-editar">Editar</button>
                        <button class="btn btn-small btn-danger btn-excluir">Excluir</button>
                    </div>
                </article>
            `;
